...
namespace secJoin {
    ...
    inline void fillInplace(span<u8> val, u64 index, bool bit)
    {
        ...
        if (o) {
            u8 mask = ~u8(0) << (o + 1);
            val[i] = ((bit) * (val[i] | mask)) | ((!bit) * (val[i] & ~mask));
            ++i;
        }
    ...
    inline i64 signExtend(i64 val, u64 bit)
    {
        ...
        return ((sign) * (val | mask)) | ((!sign) * (val & ~mask));
    }
    ...
    struct is_container<
        ...
        coproto::void_t<
            ...
            coproto::enable_if_t<coproto::has_size_member_func<typename std::remove_reference<Container>::type>::value>>> : coproto::true_type {};
    ...
    auto asSpan(T &&t)
    {
        ...
        if constexpr (is_container<T>::value) {
            ...
            static_assert(is_container<T>::value || std::is_trivial_v<std::remove_reference_t<T>>);
        }
    }
