...
namespace secJoin {
    ...
    inline void perfectUnshuffle(TBinMatrix &in, TBinMatrix &out0, TBinMatrix &out1, u64 dstShift, u64 numEntries)
    {
        if (dstShift % 8)
            throw RTE_LOC;
    ...
    // `gen`.
    void AggTree::init(u64 n, u64 bitsPerEntry, Type type, const Operator &op, CorGenerator &gen)
    {
        computeTreeSizes(n);
    ...
    // written to `dst`.
    macoro::task<> AggTree::apply(const BinMatrix &src, const BinMatrix &controlBits, coproto::Socket &comm, PRNG &prng, BinMatrix &dst)
    {
        auto root = Level{};
    ...
    oc::BetaCircuit AggTree::upstreamCir(u64 bitsPerEntry, Type type, const Operator &add)
    {
        oc::BetaCircuit cir;

        oc::BetaBundle leftPreVal(bitsPerEntry), leftSufVal(bitsPerEntry), rghtPreVal(bitsPerEntry), rghtSufVal(bitsPerEntry), leftPreBit(1), leftSufBit(1),
            rghtPreBit(1), rghtSufBit(1), prntPreVal(bitsPerEntry), prntSufVal(bitsPerEntry), prntPreBit(1), prntSufBit(1), temp1(bitsPerEntry),
            temp2(bitsPerEntry);
    ...
    macoro::task<> AggTree::upstream(
        const BinMatrix &src, const BinMatrix &controlBits, coproto::Socket &comm, PRNG &prng, Level &root, span<SplitLevel> levels)
    {
    ...
    oc::BetaCircuit AggTree::leafCircuit(u64 bitsPerEntry, const Operator &op, Type type)
    {
        ...
        oc::BetaCircuit cir;
        oc::BetaBundle leftVal(bitsPerEntry), leftPreVal(bitsPerEntry), leftSufVal(bitsPerEntry), leftPreBit(1), leftSufBit(1), leftOut(bitsPerEntry),

            rghtVal(bitsPerEntry), rghtPreVal(bitsPerEntry), rghtSufVal(bitsPerEntry), rghtPreBit(1), rghtSufBit(1), rghtOut(bitsPerEntry),

            lt1(bitsPerEntry), lt2(bitsPerEntry), lt3(bitsPerEntry),

            rt1(bitsPerEntry), rt2(bitsPerEntry), rt3(bitsPerEntry);
    ...
    //	// apply the downstream circuit to each level of the tree.
    macoro::task<> AggTree::computeLeaf(SplitLevel &leaves, SplitLevel &preSuf, BinMatrix &dst, PRNG &prng, coproto::Socket &comm)
    {
        {
