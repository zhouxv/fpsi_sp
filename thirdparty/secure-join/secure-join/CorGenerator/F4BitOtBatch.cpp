...
namespace secJoin {
    ...
    F4BitOtBatch::F4BitOtBatch(GenState *state, bool sender, oc::Socket &&s, PRNG &&p)
        : Batch(state, std::move(s), std::move(p)), mSendRecv(std::in_place_type_t<SendBatch>{})
    {
        if (sender)
            mSendRecv.emplace<0>();
        else
    ...
    void F4BitOtBatch::getCor(Cor *c, u64 begin, u64 size)
    {
        ...
        mSendRecv | match{ [&](SendBatch &send) {
    ...
    void F4BitOtBatch::setBase(BaseCor &base)
    {
        ...
        mSendRecv |
            match{ [&](SendBatch &send) {
                      send.mDelta = base.mVoleDelta;
                      send.mSender.setSilentBaseOts(base.getSendOt(send.mSender.silentBaseOtCount()), base.getSendVole(send.mSender.baseVoleCount()));
                  },
                   [&](RecvBatch &recv) {
                       recv.mReceiver.setSilentBaseOts(base.getRecvOt(recv.mReceiver.silentBaseOtCount()), base.getRecvVole(recv.mReceiver.baseVoleCount()));
                   } };
    }

    macoro::task<> F4BitOtBatch::getTask(BatchThreadState &threadState)
    {
        return mSendRecv | match{ [&](SendBatch &send) { return send.sendTask(mGenState, mIndex, mSize, mPrng, mSock, mCorReady, threadState); },
                                  [&](RecvBatch &recv) {
    ...
    macoro::task<> F4BitOtBatch::RecvBatch::recvTask(
        GenState *state, u64 batchIdx, u64 size, PRNG &prng, oc::Socket &sock, macoro::async_manual_reset_event &corReady, BatchThreadState &threadState)
    {
    ...
    void F4BitOtBatch::RecvBatch::compressRecver(span<oc::block> A)
    {
        mOts.resize(A.size());
        ...
        for (u64 i = 0; i < A.size(); i += 16) {
            ...
            for (u64 j = 0; j < 2; ++j) {
                ...
                *lsbIter8++ = lsb[0] ^ (lsb[1] << 1) ^ (lsb[2] << 2) ^ (lsb[3] << 3) ^ (lsb[4] << 4) ^ (lsb[5] << 5) ^ (lsb[6] << 6) ^ (lsb[7] << 7);
                ...
                // pack the choice bits.
                *msbIter8++ = (msb[0] >> 1) ^ (msb[1]) ^ (msb[2] << 1) ^ (msb[3] << 2) ^ (msb[4] << 3) ^ (msb[5] << 4) ^ (msb[6] << 5) ^ (msb[7] << 6);
    ...
    void F4BitOtBatch::SendBatch::compressSender(span<oc::block> B)
    {
        for (u64 i = 0; i < 4; ++i)
            mOts[i].resize(B.size());

        std::array<u16 *, 4> iters{ (u16 *)mOts[0].data(), (u16 *)mOts[1].data(), (u16 *)mOts[2].data(), (u16 *)mOts[3].data() };
