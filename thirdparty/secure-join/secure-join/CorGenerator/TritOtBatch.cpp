...
namespace secJoin {
    ...
    TritOtBatch::TritOtBatch(GenState *state, bool sender, oc::Socket &&s, PRNG &&p) : Batch(state, std::move(s), std::move(p))
    {
        if (sender)
            mSendRecv.emplace<0>();
        else
    ...
    void TritOtBatch::setBase(BaseCor &base)
    {
        ...
        mSendRecv | match{ [&](SendBatch &send) { send.mSender.setSilentBaseOts(base.getSendOt(send.mSender.silentBaseOtCount())); },
                           [&](RecvBatch &recv) {
    ...
    void TritOtBatch::RecvBatch::compressRecver(span<oc::block> A)
    {
        if (A.size() % 128)
            throw RTE_LOC;
        ...
        for (u64 i = 0; i < A.size(); i += 8) {
            ...
            *choiceIter8++ =
                choice[0] ^ (choice[1] << 1) ^ (choice[2] << 2) ^ (choice[3] << 3) ^ (choice[4] << 4) ^ (choice[5] << 5) ^ (choice[6] << 6) ^ (choice[7] << 7);
    ...
    void TritOtBatch::SendBatch::compressSender(block delta, span<oc::block> B)
    {
        if (B.size() % 128)
            throw RTE_LOC;
        ...
        std::array<u8 *, 2> lsbIter{ (u8 *)mLsb[0].data(), (u8 *)mLsb[1].data() };
        std::array<u8 *, 2> msbIter{ (u8 *)mMsb[0].data(), (u8 *)mMsb[1].data() };
