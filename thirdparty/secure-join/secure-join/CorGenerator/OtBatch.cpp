...
namespace secJoin {
    OtBatch::OtBatch(GenState *state, bool sender, oc::Socket &&s, PRNG &&p) : Batch(state, std::move(s), std::move(p))
    {
        if (sender) {
            mSendRecv.emplace<0>();
        } else {
    ...
    {
        mSendRecv | match{ [&](SendOtBatch &send) {
    ...
    void OtBatch::setBase(BaseCor &base)
    {
        ...
        mSendRecv | match{ [&](SendOtBatch &send) { send.mSender.setSilentBaseOts(base.getSendOt(send.mSender.silentBaseOtCount())); },
                           [&](RecvOtBatch &recv) {
    ...
    macoro::task<> OtBatch::getTask(BatchThreadState &threadState)
    {
        ...
        if (mGenState->mMock) {
            ...
            t = mSendRecv | match{ [&](SendOtBatch &send) {
    ...
    void OtBatch::clear()
    {
        mSendRecv | match{ [&](SendOtBatch &send) { send.mMsg2 = {}; },
                           [&](RecvOtBatch &recv) {
                               recv.mChoice = {};
                               recv.mMsg = {};
                           } };
