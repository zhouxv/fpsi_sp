...
namespace secJoin {
    ...
    struct AltModKeyMultSender {
        ...
        void init(CorGenerator &ole, std::optional<AltModPrf::KeyType> keyShare = {}, span<const std::array<block, 2>> keyOts = {})
        {
            if (keyOts.size() == 0) {
        ...
        // an F3 sharing.
        macoro::task<> mult(
            oc::MatrixView<const block> xLsb, oc::MatrixView<const block> xMsb, oc::Matrix<block> &xkLsb, oc::Matrix<block> &xkMsb, coproto::Socket &sock);

        // This function allows setting the OTs corresponding to the
        // other party's key. Additionally, if they key is shared
        // then the local key share is provided.
        void setKeyOts(std::optional<AltModPrf::KeyType> keyShared, span<const std::array<block, 2>> ots)
        {
    ...
    struct AltModKeyMultReceiver {
        ...
        void init(CorGenerator &ole, std::optional<AltModPrf::KeyType> key, span<const block> keyOts)
        {
        ...
        // If x if F2 shared, xMsb should be empty. The result is always
        // an F3 sharing.
        macoro::task<> mult(u64 n, oc::Matrix<block> &xk0, oc::Matrix<block> &xk1, coproto::Socket &sock);

        // sets the key to be used and the OTs corresponding to it.
        // the choice bits of `ots` must be `k`.
        void setKeyOts(AltModPrf::KeyType k, span<const block> ots)
        {
            if (ots.size() != AltModPrf::KeySize)
                throw RTE_LOC;
            mKey = k;
