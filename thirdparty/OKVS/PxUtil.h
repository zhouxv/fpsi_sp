...
namespace volePSI {
    ...
    struct PxVector {
        ...
        {}
        PxVector(PxVector &&v) : mOwning(std::move(v.mOwning)), mElements(std::move(v.mElements))
        {}

        PxVector(span<value_type> e) : mElements(e)
        {}

        PxVector(u64 size) : mOwning(new value_type[size]), mElements(mOwning.get(), size)
        {}

        // return a iterator to the i'th element. Should be pointer symmatics
        inline iterator operator[](u64 i)
        {
            return &mElements[i];
        }
        ...
        struct Helper {
            ...
            inline static mut_value_type newElement()
            {
                return {};
            }

            // return the iterator for the return type of newElement().
            mut_iterator asPtr(mut_value_type &t)
            {
                return &t;
            }
            ...
            inline static auto iterPlus(const_iterator p, u64 i)
            {
                return p + i;
            }

            // return the iterator plus the given number of rows
            inline static auto iterPlus(mut_iterator p, u64 i)
            {
                return p + i;
            }
    ...
    struct PxMatrix {
        ...
        PxMatrix() = default;
        PxMatrix(const PxMatrix &v) : mElements(v.mElements), mRows(v.mRows), mCols(v.mCols)
        {}

        PxMatrix(PxMatrix &&v) = default;

        PxMatrix(MatrixView<value_type> e) : mElements(e.data(), e.size()), mRows(e.rows()), mCols(e.cols())
        {}

        PxMatrix(u64 rows, u64 cols) : mOwning(new value_type[rows * cols]), mElements(mOwning.get(), rows * cols), mRows(rows), mCols(cols)
        {}

        PxMatrix(iterator ptr, u64 rows, u64 cols) : mElements(ptr, rows * cols), mRows(rows), mCols(cols)
        {}
        ...
        struct Helper {
            ...
            // return the iterator for the return type of newElement().
            iterator asPtr(std::vector<value_type> &t)
            {
                return t.data();
            }
            ...
            // return the iterator plus the given number of rows
            inline auto iterPlus(const_iterator p, u64 i)
            {
                return p + i * mCols;
            }

            // return the iterator plus the given number of rows
            inline auto iterPlus(mut_iterator p, u64 i)
            {
                return p + i * mCols;
            }
