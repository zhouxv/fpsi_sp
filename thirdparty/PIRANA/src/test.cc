...
void test_pir_correctness(Server &server, std::vector<std::vector<uint64_t>> &answer, uint32_t index, PirParms &pir_parms)
{
    ...
    uint64_t left_rotate_slot = std::ceil(static_cast<double>(num_slot) / pir_parms.get_pre_rotate());
    for (uint32_t i = 0; i < answer.size() && count < num_slot; i++) {
        auto row_index = index / half_N;
        auto offset = (index - std::min(left_rotate_slot, pir_parms.get_rotate_step())) % half_N;
        for (uint32_t j = 0; j < pir_parms.get_rotate_step() && count < num_slot; j++) {
            for (uint32_t k = 0; k < pir_parms.get_pre_rotate() / 2 && count < num_slot; k++) {
                uint32_t index = row_index * half_N + (k * pir_parms.get_rotate_step() + j + offset) % half_N;
            ...
            for (uint32_t k = 0; k < pir_parms.get_pre_rotate() / 2 && count < num_slot; k++) {
                uint32_t index = (row_index + 1) * half_N + (k * pir_parms.get_rotate_step() + j + offset) % half_N;
                auto response_item = answer[i][index % N];
                auto plain_item = real_item[count];
        ...
        if (pir_parms.get_is_compress() == false) {
            ...
            for (uint32_t i = 0; i < num_ct; i++) {
                ...
                if (real_item.at(i) != answer.at(bundle_size * i + bundle_index).at(slot_index)) {
                    ...
                    std::cout << q << std::endl;
                    std::cout << i << std::endl;
                    std::cout << real_item.at(i) << " " << answer.at(bundle_size * i + bundle_index).at(slot_index) << std::endl;
                }
            }
        } else {
            auto num_slot = pir_parms.get_num_slot();
            for (uint32_t i = 0, slot = 0, ct_index = 0; i < real_item.size(); i++, slot++) {
                if (slot == num_slot) {
                    slot = 0;
                    ct_index++;
                }
                if (real_item.at(i) == answer.at(ct_index).at(slot + loc * num_slot)) {
                    std::cout << i << " " << real_item.at(i) << " " << answer.at(ct_index).at(slot + loc * num_slot) << std::endl;
                }
            }
        }
    }
...
void test_mulptiply(
    SEALContext &context, EncryptionParameters &parms, Encryptor &encryptor, Decryptor &decryptor, BatchEncoder &batch_encoder, uint64_t run_times)
{
    ...
    cout << "Multiply " << run_times << " , Time: " << multiply_time << " ms" << endl;
}
