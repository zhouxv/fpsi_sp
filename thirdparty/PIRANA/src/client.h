...
public:
    ...
    std::vector<std::vector<uint64_t>> extract_answer(std::stringstream &responsestream);
    std::vector<std::vector<uint64_t>> extract_batch_answer(std::stringstream &responsestream);
    void getBlockFromAnswer(std::vector<oc::block> &blocks, std::vector<std::vector<uint64_t>> &answer, std::vector<uint32_t> &query);
