...
void try_clear_irrelevant_bits(const seal::EncryptionParameters &parms, seal::Ciphertext &ciphertext)
{
...
std::pair<uint32_t, uint32_t> get_cw_code_k2(const uint64_t number, const uint64_t encoding_size)
{
    ...
    for (int64_t pointer = encoding_size - 1; pointer >= 0 && k_prime > 0; pointer--) {
...
std::vector<uint32_t> get_perfect_constant_weight_codeword(uint64_t number, uint64_t encoding_size, uint64_t hamming_weight)
{
    ...
    for (int32_t pointer = encoding_size - 1; pointer >= 0 && k_prime > 0; pointer--) {
        if (remainder >= choose(pointer, k_prime)) {
...
uint64_t choose_rotate_parms(const uint64_t &N, const uint64_t &num_payload_slot, const uint64_t &num_selection_vector)
{
    uint64_t rotate_expand = 1;
    uint64_t min_rotation = INT64_MAX;
    uint64_t num_output_cipher = std::ceil(num_payload_slot * 1.0 / N);
    while (1) {
        auto total_rot_times =
            (rotate_expand - 1) * num_selection_vector + (uint64_t)(std::ceil((double)std::min(N, num_payload_slot) / rotate_expand) - 1) * num_output_cipher;
    ...
    seal::MemoryPoolHandle pool)
{
    ...
    SEAL_ITERATE(seal::util::iter(size_t(0)), dest_size, [&](auto I) {
        ...
        auto behz_ciphertext_product = [&](seal::util::ConstPolyIter in1_iter,
                                           seal::util::ConstPolyIter in2_iter,
                                           seal::util::ConstModulusIter base_iter,
                                           size_t base_size,
                                           seal::util::PolyIter out_iter) {
