...
public:
    ...
    Server(PirParms &pir_parms, bool is_batch, std::vector<oc::block> &raw_db);
    void gen_random_db();
    void read_db(std::vector<oc::block> &raw_db);
    void encode_to_ntt_db();
    void batch_encode_to_ntt_db_without_compress();
    void batch_encode_to_ntt_db_with_compress();

    std::vector<seal::Ciphertext> load_query(std::stringstream &query_stream, uint32_t query_ct_size);
    std::vector<seal::Ciphertext> gen_selection_vector(std::vector<seal::Ciphertext> &query);

    std::vector<seal::Ciphertext> gen_selection_vector_batch(std::vector<seal::Ciphertext> &query);

    std::vector<seal::Ciphertext> rotate_selection_vector(const std::vector<seal::Ciphertext> &selection_vectors);

    std::vector<seal::Ciphertext> mul_database_with_compress(const std::vector<seal::Ciphertext> &rotated_selection_vectors);

    std::stringstream inner_product(const std::vector<seal::Ciphertext> &selection_vector);
