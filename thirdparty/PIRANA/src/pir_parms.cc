...
PirParms::PirParms(const uint64_t num_payloads, const uint64_t payload_size) : _payload_size(payload_size), _num_query(1)
{
    ...
    if (std::floor(poly_degree / num_payloads)) {
        ...
        _pre_rotate = std::max(_pre_rotate, uint64_t(std::floor(poly_degree * 1.0 / num_payloads)));
    }
...
void PirParms::set_seal_parms(uint64_t poly_degree, std::vector<int> coeff_modulus, uint64_t prime_len)
{
    _seal_parms = seal::EncryptionParameters(seal::scheme_type::bfv);
    _seal_parms.set_poly_modulus_degree(poly_degree);
    _seal_parms.set_coeff_modulus(seal::CoeffModulus::Create(poly_degree, coeff_modulus));
    _seal_parms.set_plain_modulus(seal::PlainModulus::Batching(poly_degree, prime_len));
}
...
void PirParms::print_seal_parms()
{
    ...
    std::cout << "|   poly_modulus_degree: " << _seal_parms.poly_modulus_degree() << std::endl;
    /*
      For the BFV scheme print the plain_modulus parameter.
      */
    if (_seal_parms.scheme() == seal::scheme_type::bfv) {
        std::cout << "|   plain_modulus: " << _seal_parms.plain_modulus().value() << std::endl;
    }
    /*
...
// num_slot
// bundle_size
void PirParms::get_all_index_hash_result(const uint64_t num_payloads, const uint64_t num_query, const double cuckoo_factor)
{
    std::cout << "--------------------------------------" << std::endl;
    ...
    if (_is_compress == false) {
        ...
        _bundle_size = 0;
        while (_bundle_size * N < static_cast<uint32_t>(num_query * cuckoo_factor)) {
            _bundle_size++;
        }
    ...
    _table = std::make_shared<kuku::KukuTable>(_table_size, stash_size, hash_count, hash_seed, max_probe, empty_item);
    ...
    for (uint64_t index = 0; index < num_payloads; index++) {
        ...
        for (auto &position : result) {
            ...
            if (_hash_index.count(std::to_string(index * _table_size + position)) != 0) {
                std::cout << "hash string error: " << index << std::endl;
            }
            assert(_hash_index.count(std::to_string(index * _table_size + position)) == 0);
            _hash_index[std::to_string(index * _table_size + position)] = _bucket[position].size() - 1;
        }
    }
...
PirParms::PirParms(
    const uint64_t num_payloads, const uint64_t payload_size, const uint64_t num_query, const bool is_batch, const bool is_compress, const bool enable_rotate)
    : _num_payloads(num_payloads), _payload_size(payload_size), _num_query(num_query), _is_compress(is_compress), _enable_rotate(enable_rotate)
{
...
void PirParms::print_pir_parms()
{
    ...
    std::cout << "|   Payload size (|pl|): " << _payload_size << " Bytes" << std::endl;
