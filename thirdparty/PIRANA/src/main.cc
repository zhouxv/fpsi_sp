...
void batch_pir_main(const uint64_t num_payloads, const uint64_t payload_size, const uint64_t num_query, const bool is_batch, const bool is_compress)
{
    std::cout << "Start batch PIR! " << std::endl;
    PirParms pir_parms(num_payloads, payload_size, num_query, is_batch, is_compress);
    ...
    timer.reset();
    std::vector<std::vector<uint64_t>> answer = batch_client.extract_batch_answer(response);
    auto extract_time = timer.elapsed();
    ...
    test_batch_pir_correctness(batch_server, answer, batch_query_index, pir_parms);
}
...
int main(int argc, char *argv[])
{
    ...
    while (1) {
        int option_index = 0;
        static struct option long_options[] = { { "num_payload", optional_argument, NULL, 'n' }, { "payload_size", optional_argument, NULL, 'x' },
                                                { "num_query", optional_argument, NULL, 'l' },   { "is_batch", optional_argument, NULL, 'b' },
                                                { "is_compress", optional_argument, NULL, 'c' }, { NULL, 0, NULL, 0 } };
    ...
    if (is_batch == true) {
        batch_pir_main(num_payloads, payload_size, num_query, is_batch, is_compress);
    } else {
